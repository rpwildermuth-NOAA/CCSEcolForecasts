...
SDR.loo.eval <-EvalLOO(DataInput=obs, gbm.x=varNames, gbm.y=targetName,lr=lr, tc=tc)

# test with 25% of the full dataset
SDR.7525.eval <- evalPct(DataInput= obs,gbm.x=varNames, gbm.y=targetName,lr=lr, tc=tc, retainPct = 0.75)

# compare to full model fit 
#!!RW: Is this just the same as buildSDM()?
SDR.100.eval <- evalPct(DataInput= obs,gbm.x=varNames, gbm.y=targetName,lr=lr, tc=tc, retainPct = 1)
